...
const KanbanBoard = () => {
  ...
  const handleDragStart = (
    ...
    try {
      // store id + source so drop can recover even if state is lost
      e.dataTransfer.setData(
        "application/json",
        JSON.stringify({ id: email.id, source: sourceColumn })
      );
    } catch (err) {
  ...
  const handleDrop = (e: React.DragEvent, targetColumnId: string) => {
    e.preventDefault();
    // try to get drag info first from React state, then fallback to dataTransfer
    let drag = draggedEmail;
    if (!drag) {
      try {
        const raw =
        ...
        if (raw) {
          ...
          const found = emails.find((em) => em.id === parsed.id);
          if (found) {
            drag = { email: found, sourceColumn: parsed.source };
          }
        }
      } catch (err) {
        // ignore parse errors
    ...
    // create a shallow but safe copy of columns and their email arrays
    const newColumns = {
