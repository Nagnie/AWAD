...
const KanbanBoard = () => {
    ...
    const inboxEmails = emails.filter((email: EmailMessage) => !processedEmailIds.has(email.id));
    ...
    useEffect(() => {
        ...
        setColumns((prev) => {
            const isSame = Object.keys(newColumns).every((key) => {
                const prevIds = (prev as any)[key].emails.map((e: EmailMessage) => e.id).join(",");
        ...
        if (!alreadyInTarget) {
            const updatedEmail = {
                ...
                labelIds: (email.labelIds || [])
                    .filter((label: string) => !["TODO", "IN_PROGRESS", "DONE"].includes(label))
